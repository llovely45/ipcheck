...
-   **🛡️ 多源 IP 纯净度**：后台并行查询无需密钥且支持 CORS 的公开接口，客户页面只展示 1–100 综合分和风控点数量，不展示供应商逐项对比；缺失来源不会被填充为虚构分数。
...
-   **纯净度评分口径**：1 为最差，100 为最好。综合分为可用来源的加权平均。页面的“风控点”是去重后的风险标记数量，并合并地理数据中的 Proxy/Bogon 标记；各接口的标记只代表对应数据源的观察结果。
